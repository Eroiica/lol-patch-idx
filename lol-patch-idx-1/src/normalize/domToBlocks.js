...
function domToBlocks($) {
  ...
  $root.find("*").each((_, el) => {
    ...
    // Skip structural wrappers and inline elements
    if (["div", "section", "article", "main", "nav", "header", "footer",
         "script", "style", "img", "a", "span", "em", "strong", "b", "i",
         "br", "svg", "picture", "source", "figure", "figcaption"].includes(tag)) {
      return;
    }
...
function markdownToBlocks(md) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (numMatch) {
      blocks.push({ kind: "listItem", text: cleanText(numMatch[1]) });
      continue;
    }
