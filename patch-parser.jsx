...
const StatRow = ({ stat, before, after, delta }) => (
  <div style={{
    ...
    <span style={{ color: "#A09070", fontFamily: "'IBM Plex Sans', sans-serif", fontSize: 12 }}>{stat}</span>
    <span style={{ color: "#ef4444", textDecoration: "line-through", opacity: 0.6 }}>{before}</span>
    <span style={{ color: "#4ade80" }}>
      {after}
      {delta && <span style={{ color: "#facc15", marginLeft: 6, fontSize: 10 }}>({typeof delta === 'number' ? (delta > 0 ? `+${delta}` : delta) : delta})</span>}
    </span>
  </div>
);
  ...
  return (
    ...
    >
      ...
      {open && (
        <div style={{ padding: "0 18px 16px", borderTop: "1px solid rgba(200,170,110,0.06)" }}>
          <div style={{
            display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, padding: "8px 0 2px",
            fontSize: 10, fontFamily: "'JetBrains Mono', monospace", color: "#5A4F3C", textTransform: "uppercase", letterSpacing: "0.1em"
          }}>
            <span>Stat</span><span>Before</span><span>After</span>
          </div>
