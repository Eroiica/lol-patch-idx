...
function parseStatLine(text) {
  ...
  if (beforeSlash && afterSlash && beforeSlash.length === afterSlash.length) {
    const series = buildAlignedSeries(label, beforeRaw, afterRaw, beforeSlash, afterSlash);
    
    // Check if base values are identical — the real change may be in parenthetical ratios
    const identical = JSON.stringify(beforeSlash) === JSON.stringify(afterSlash);
    if (identical) {
      const bNotes = extractParenNotes(beforeRaw);
      const aNotes = extractParenNotes(afterRaw);
      if (bNotes.length && aNotes.length && bNotes[0] !== aNotes[0]) {
        // Try to parse the parenthetical as its own aligned series
        const ratioB = splitSlashValues(bNotes[0]);
        const ratioA = splitSlashValues(aNotes[0]);
        if (ratioB && ratioA && ratioB.length === ratioA.length) {
          const ratioSeries = buildAlignedSeries(label + " (ratio)", bNotes[0], aNotes[0], ratioB, ratioA);
          return [series, ratioSeries]; // Return both
        }
        // Fallback: emit ratio as text change
        return [series, { stat: label + " (ratio)", type: "text", before: { value: bNotes[0] }, after: { value: aNotes[0] } }];
      }
    }
    return series;
  }
